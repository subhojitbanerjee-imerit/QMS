...
async function getAdminSheetsAccessToken(): Promise<string> {
  ...
  if (!clientId || !clientSecret || !refreshToken) {
    throw new Error("Backend Sheets sync is not configured. Set GOOGLE_OAUTH_CLIENT_ID, GOOGLE_OAUTH_CLIENT_SECRET, and GOOGLE_OAUTH_REFRESH_TOKEN.");
  }
  ...
  if (!tokenResponse.ok) {
    ...
    throw new Error(`Google OAuth refresh-token request failed (${tokenResponse.status}): ${detail}`);
  }
